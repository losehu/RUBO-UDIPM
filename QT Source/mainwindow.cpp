...
void MainWindow::open_mainwindows_updata_windows() {
    ...
    int n1 = screenX / 5 * 3 / init_width;
    int n2 = screenY / 5 * 3 / init_high;
    pic.zoom = min(n1, n2);
...
void MainWindow::init_windows(int start_width, int start_high) {
    ...
    setFixedSize(start_width + 260, max(start_high + 50, 550));
    int model5_start_x = 0, model5_start_y = start_high + 5;
    init_model5_windows(model5_start_x, model5_start_y);
}
...
void MainWindow::on_button_openpic_clicked() {
    ...
    string path;
    QString_to_string(filename, &path);
    read_pic(path);
    update_windows();
    int n1 = screenX / 5 * 3 / img.width();
    int n2 = screenY / 5 * 3 / img.height();
    pic.width = img.width();
    pic.high = img.height();
    pic.zoom = min(n1, n2);
