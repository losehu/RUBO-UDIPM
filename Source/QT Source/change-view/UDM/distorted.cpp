...
int UdmWindow:: get_distorted_mat(string pic_name[], string pic_name_ok[], int pic_num, int conor_width, int conor_high,
                      double parameter[]) {
    int ok_pic = 0;
       ...
       for (int image_count = 0; image_count < pic_num; image_count++) {
           ...
           if (image_count == 0)  //读入第一张图片时获取图像宽高信息
           {
               ...
               udm_pic.pic_width=imageInput.cols;
               udm_pic.pic_high=imageInput.rows;
...
UdmWindow::on_pushButton_openpic_clicked(1);
       return ok_pic;
...
string out_mat="double cameraMatrix[3][3]=";
for(int i=0; i<cameraMatrix.rows; i++){
    if(i==0) out_mat += "{{";
   for(int j=0; j<cameraMatrix.cols; j++)
{
       out_mat += to_string(cameraMatrix.at<double>(i,j));
       if(j!=cameraMatrix.cols-1)out_mat+= ",";
   }
  if(i!=cameraMatrix.rows-1) out_mat += "},{";
   else out_mat += "}};";
}
out_mat+="\ndouble distCoeffs[5]=";
...
{
       out_mat += to_string(distCoeffs.at<double>(i,j));
       if(j!=distCoeffs.cols-1)out_mat+= ",";
   }
  if(i!=distCoeffs.rows-1) out_mat += "},{";
   else out_mat += "};";
}

out_mat+="\nint move_xy[2]={";
        out_mat+=to_string(udm_pic.move_x);out_mat+=",";
        out_mat+=to_string(udm_pic.move_y);out_mat+="};";
